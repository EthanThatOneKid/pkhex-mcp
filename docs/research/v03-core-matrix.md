...
- **fog** — the bytes/storage are reachable but the *meaning* lives elsewhere
...
5. **Active-slot selection.** Core resolves it internally during load —
   `SAV4` constructor picks General/Storage partitions via
...
- Money @ active-slot `Trainer1+0x14`: **91,124** ✓ (player truth; inactive
  slot reads 89,332)
- Badges byte: `0b00010011` ✓ · map id 120 ✓ · dex magic `0xBEEFCAFE` ✓
- Dex popcount with Zukan4 bit math masked to ≤493: **41 caught / 102 seen** ✓
- Bag head via PlayerBag4Pt base 0x630: Repel ×2 (id 79), Yellow Shard ×2
  (id 74), Escape Rope ×1 (id 78) ✓ — spike ground truth reproduced
