...
2. **NDS BIOS + firmware files** placed in BizHawk's `Firmware` folder:
   - `biosnds7.bin` (ARM7 BIOS, 16 KB)
   - `biosnds9.bin` (ARM9 BIOS, 4 KB)
   - `firmware.dfc` (DS firmware image)
   These must come from your own Nintendo DS (see BizHawk's NDS docs).
3. **Pokémon Platinum (US)** ROM dump — gamecode `CPUE`.
4. The pkhex-mcp desktop app running (`GET /state` reachable).
...
3. `Tools ▸ Lua Console` → `Add New Script…` → select
   `bridge/platinum-sync.lua` from this repo.
